...
COMPLETE_HISTORICAL_DATA = {
   "SET_1": [2, 17, 20, 28, 33, 49],
"SET_2": [2, 18, 19, 26, 28, 41],
"SET_3": [6, 10, 12, 29, 33, 57],
"SET_4": [4, 23, 28, 38, 39, 49],
"SET_5": [1, 8, 31, 32, 43, 47],
"SET_6": [5, 12, 23, 39, 50, 57],
"SET_7": [2, 18, 28, 43, 50, 52],
"SET_8": [1, 9, 11, 19, 44, 45],
"SET_9": [20, 22, 23, 27, 38, 51],
"SET_10": [7, 15, 23, 34, 54, 58],
"SET_11": [3, 12, 20, 23, 30, 31],
"SET_12": [1, 9, 15, 23, 39, 57],
"SET_13": [14, 17, 22, 29, 34, 36],
"SET_14": [24, 34, 36, 37, 41, 48],
"SET_15": [15, 16, 41, 42, 47, 57],
"SET_16": [4, 14, 15, 16, 23, 35],
"SET_17": [7, 24, 27, 30, 33, 52],
"SET_18": [12, 24, 25, 29, 38, 41],
"SET_19": [12, 17, 18, 33, 41, 57],
"SET_20": [8, 16, 22, 36, 38, 41],
"SET_21": [1, 12, 16, 19, 46, 54],
"SET_22": [9, 20, 23, 32, 39, 44],
"SET_23": [18, 27, 39, 42, 49, 52],
"SET_24": [5, 19, 30, 43, 50, 51],
"SET_25": [23, 38, 39, 40, 42, 50],
"SET_26": [5, 7, 18, 28, 32, 48],
"SET_27": [3, 7, 10, 13, 15, 44],
"SET_28": [12, 17, 37, 44, 45, 46],
"SET_29": [21, 22, 37, 39, 48, 50],
"SET_30": [4, 6, 15, 41, 43, 52],
"SET_31": [3, 13, 15, 30, 47, 57],
"SET_32": [14, 22, 33, 39, 50, 57],
"SET_33": [4, 5, 7, 24, 34, 47],
"SET_34": [2, 17, 26, 28, 54, 57],
"SET_35": [7, 15, 29, 40, 43, 52],
"SET_36": [1, 16, 27, 34, 44, 57],
"SET_37": [28, 29, 35, 42, 43, 56],
"SET_38": [4, 16, 31, 35, 36, 50],
"SET_39": [1, 11, 19, 27, 41, 57],
"SET_40": [14, 18, 43, 45, 50, 52],
}
...
PRESSURE_WEIGHTS = {0: 5, 1: 5, 2: 3, 3: 3, 4: 1, 5: 1, 6: 1}
WAVE_WEIGHT = 1
RECENCY_WEIGHT = 0.5
BALANCE_WEIGHT = 0.5
LOOKBACK = 10
...
# ===========================
def compute_pressure_score(number, available_numbers):
    """Return pressure score based on available_numbers"""
    return PRESSURE_WEIGHTS.get(available_numbers.get(number, 0), 1)

def compute_wave_score(number, position_index, recent_sets):
    """Simple wave score: lower score if number appeared heavily recently"""
    recent_count = 0
    for s in recent_sets[-LOOKBACK:]:
        if len(s) > position_index and s[position_index][0] == number:
            recent_count += 1
    if recent_count >= 3:
        return 1  # overused
    elif recent_count == 2:
        ...
        return 5  # cool number

def compute_recency_adjustment(number, recent_sets):
    """Penalize if number appeared in last 2 sets"""
    if not recent_sets:
        return 0
    last_sets = recent_sets[-2:]
    count = sum(1 for s in last_sets for n, _ in s if n == number)
    return -count

def compute_balance_adjustment(number, selected_numbers):
    """Bonus for balancing odds/evens or avoiding clustering"""
    if not selected_numbers:
        return 0
    last_num = selected_numbers[-1]
    if (last_num % 2) != (number % 2):
        return 1  # alternating odd/even
    return 0

def analyze_enhanced_prediction(actual_sets, available_numbers):
    """Enhanced prediction with pressure, wave, recency, and balance scoring"""
    final_set = []
    candidate_scores = defaultdict(list)

    for pos_index, pos_name in enumerate(POSITION_RANGES.keys()):
        candidates = [n for n in available_numbers.keys()
                     if n in POSITION_RANGES[pos_name] and available_numbers[n] >= 0]

        scored_candidates = []

        for n in candidates:
            pressure_score = compute_pressure_score(n, available_numbers)
            wave_score = compute_wave_score(n, pos_index, actual_sets)
            recency_adj = compute_recency_adjustment(n, actual_sets)
            balance_adj = compute_balance_adjustment(n, final_set)

            total_score = (pressure_score +
                          WAVE_WEIGHT * wave_score +
                          RECENCY_WEIGHT * recency_adj +
                          BALANCE_WEIGHT * balance_adj)
        ...
        candidate_scores[pos_name] = scored_candidates[:3]

        final_number = scored_candidates[0]['number'] if scored_candidates else None
        final_set.append(final_number)

    return final_set, candidate_scores

# ===========================
# PARSERS FOR ENHANCED PREDICTION
# ===========================
def parse_previous_set(raw_line):
    """Parse a line like '[1 = 1], [6 = 1], ...' into dict {number: count}"""
    result = {}
...
def parse_current_availability(raw_text):
    """Parse space-separated 'number = count' pairs"""
    result = {}
...
def run_complete_analysis(historical_data, window_id):
    ...
    class NumberPredictor:
        ...
        def validate_actual_set(self, actual_key, actual_set, window_id, predicted_pos_means, pre_EandO_set_knn):
            ...
            count_str_line = ", ".join(count_str)
            return actual_freqs, count_str_line
...
num_windows = len(keys) - window_size + 1
...
for window_idx in range(num_windows):
    window_keys = keys[window_idx : window_idx + window_size]
    window_data = {}
    ...
    if window_idx + window_size < len(keys):
        ...
        actual_freqs, count_str_line = results['num_predictor'].validate_actual_set(
            actual_next_key, actual_next_set, window_idx + 1,
            results['predicted_pos_means'], results['pre_EandO_set_knn']
        )
        next_counts_list.append(actual_freqs)
        previous_raw_lines.append(count_str_line)
...
# =====================================================
# INTEGRATED ENHANCED PREDICTION ANALYSIS
# =====================================================
print("\n" + "="*80)
print("🎯 ENHANCED PRESSURE-WAVE PREDICTION ANALYSIS")
print("="*80)

# Parse previous sets with frequency data
parsed_previous_sets = []
for raw_line in previous_raw_lines:
    parsed_set = parse_previous_set(raw_line)
    # Convert to list of tuples (number, frequency)
    set_tuples = [(num, freq) for num, freq in sorted(parsed_set.items())]
    parsed_previous_sets.append(set_tuples)

# Get current availability from last window
last_results = all_window_results[-1]
current_availability = last_results['frequency_count']

# Display available numbers table
print("\n" + "="*80)
print("✅ Available Number before the actual result show 🍀")
print("="*80)
print(f"{'':12}N1\t N2\t  N3        N4\t      N5\tN6")

# Display last several actual sets with their frequencies
display_count = min(len(previous_raw_lines), 24)
start_idx = max(0, len(previous_raw_lines) - display_count)
for i in range(start_idx, len(previous_raw_lines)):
    window_num = i + window_size + 1
    actual_key = keys[i + window_size] if (i + window_size) < len(keys) else f"SET_{window_num}"
    print(f"ACTUAL {actual_key} {previous_raw_lines[i]}")
...
print("-" * 40)
print("Available numbers based on Range Limit:")
for i in range(1, last_results['max_number'] + 1):
    print(f"{i} = {current_availability[i]}", end=" ")
    if i % 5 == 0:
        print()
print("\n" + "-" * 40)

# Run enhanced prediction
enhanced_pred, candidate_scores = analyze_enhanced_prediction(
    parsed_previous_sets,
    current_availability
)

print(f"\nAnticipated based on Enhanced Pressure-Wave Analysis Next Set (N1–N6): {enhanced_pred}")

# Display top candidates per position
print("\nTop Candidates per Position:\n")
for pos, candidates in candidate_scores.items():
    print(f"{pos}:")
    for c in candidates:
        print(f"Number={c['number']} | Total Score={c['total_score']:.1f} | "
              f"Pressure={c['pressure_score']} | Wave={c['wave_score']} | "
              f"Recency={c['recency_adj']} | Balance={c['balance_adj']}")
    print()
...
# Build history from parsed sets
history = defaultdict(list)
for s in parsed_previous_sets:
    for number, value in s:
        history[number].append(value)

def detect_trend(values):
    if len(values) < 2:
        return "Stable"
    recent = values[-3:] if len(values) >= 3 else values
    if all(v <= 1 for v in recent) and max(recent) == min(recent):
        return "Low-stable"
...
for number, values in history.items():
    ...
    likelihood = likelihood_map[trend]
    current = current_availability.get(number, 0)
    if current >= 0 and likelihood != "Avoid":
        analysis.append({
            "number": number,
            "trend": trend,
...
analysis.sort(
    key=lambda x: (rank_weight[x["likelihood"]], x["current"]),
    reverse=True
)

print("\nTREND ANALYSIS TABLE")
print("-" * 60)
for a in analysis[:20]:  # Show top 20
    print(
        f"Number {a['number']:>2} | "
        f"Trend: {a['trend']:<10} | "
        f"Likelihood: {a['likelihood']:<9} | "
        f"Current: {a['current']}"
    )

top_two_trend = [a["number"] for a in analysis[:2]]
top_six_trend = [a["number"] for a in analysis[:6]]

print("\nRECOMMENDED FIRST TWO ELEMENTS (Trend):")
print(top_two_trend)
print("\nOPTIONAL NEXT SET OF 6 ELEMENTS (Trend):")
print(top_six_trend)
...
# ===========================
print("\n" + "="*80)
print("📊 ENHANCED FINAL PREDICTIONS COMPARISON")
print("="*80)

# Add new predictions to the comparison
last_window_predictions['Enhanced Pressure-Wave'] = enhanced_pred
last_window_predictions['Trend Analysis'] = top_six_trend
...
# Recalculate consensus with new methods
all_enhanced_nums = []
for pred in last_window_predictions.values():
    if pred:
        all_enhanced_nums.extend(pred)
enhanced_consensus_freq = Counter(all_enhanced_nums)

print("\n" + "-"*60)
print("🎯 NUMBERS BY ENHANCED CONSENSUS STRENGTH")
print("-"*60)
for num, count in enhanced_consensus_freq.most_common(12):
    methods = []
    for method, pred in last_window_predictions.items():
        if pred and num in pred:
            methods.append(method[:8])
    confidence = (count / len([p for p in last_window_predictions.values() if p])) * 100
    print(f" {num:2d}: in {count}/{len([p for p in last_window_predictions.values() if p])} methods ({confidence:.0f}%) - {', '.join(methods)}")

enhanced_final_consensus = sorted([n for n, c in enhanced_consensus_freq.most_common(6)])

print("\n" + "="*80)
print(f"🌟 ENHANCED ULTIMATE CONSENSUS PREDICTION FOR SET_{len(keys) + 1}")
print("="*80)
print(f"\n {enhanced_final_consensus}")
print(f"\n Based on:")
print(f" • {num_windows} sliding window analyses")
print(f" • {len(all_predictions_by_method) + 2} prediction methods (including Enhanced Pressure-Wave & Trend)")
print(f" • Pattern learning across all windows")
print(f" • Pressure scoring, wave analysis, recency, and balance factors")
print("\n" + "="*80)
print("✅ Multi-window sliding analysis with Enhanced Prediction complete! 🍀")
print("="*80)

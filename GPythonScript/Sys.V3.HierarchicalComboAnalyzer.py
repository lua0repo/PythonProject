...
class LotteryAnalyzerUnified:
    ...
    def generate_report(self, pred, scores, ga_pred):
        ...
        for n,f,p,pr,prng,ga,tot in comp:
            lines.append(f"{n:<2} | {f:<2} | {p:<2.0f} | {pr:<2} | {prng:<2.1f} {ga:<2} | {tot:.1f}")
        lines.append("\nFINAL PREDICTION")
...
def print_set_analysis(lst):
    if not lst: return
    header = f"{'Set N#':<2} | {'Set Values':<3} | {'1st A':>2} | {'2nd A':>2} | {'Odd':>2} | {'Even':>2} | {'COdd':>2} | {'CEve':>2} | {'Total':>2}"
    sep = "="*len(header)
...
print("-" * 40)
print(f"✅ G for 20 SET⭐Ensble Mjty Vote: {ensemble_vote}")
# -------------------------------
...
print(f"✅G for 60 SET🎯 reuse Elemt here: {prediction_1st}")
print(f"✅Predicted Group Combination => {predictedPair}")
print(f"Predicted numbers : {ups_pred}")
print(f"✅G for 20/60 SET🎯⭐: {pred_set}")

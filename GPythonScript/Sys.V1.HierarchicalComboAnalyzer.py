...
# -------------------------------
historical_data = {
  "SET_1": [1, 16, 27, 34, 44, 57],
"SET_2": [28, 29, 35, 42, 43, 56],
"SET_3": [4, 16, 31, 35, 36, 50],
"SET_4": [1, 11, 19, 27, 41, 57],
"SET_5": [14, 18, 43, 45, 50, 52],
"SET_6": [22, 25, 36, 40, 49, 58],
"SET_7": [19, 20, 29, 36, 38, 47],
"SET_8": [19, 29, 31, 40, 50, 54],
"SET_9": [15, 23, 31, 38, 52, 54],
"SET_10": [22, 26, 27, 30, 46, 57],
}
...
print(f"{'Average':<10} {avg_even:<7.1f} {avg_odd:<6.1f} {avg_et:<6} {avg_ot:<6}")
print("-" * 40)
...
total_avg = round((total_et + total_ot)/num_sets)
print(f"Average sums: {avg_ot} {avg_et} total {total_avg}")
print("-" * 40)
# -------------------------------
...
def print_set_analysis(lst):
  ...
  if lst:
      avg_name = "Average"
      avg_values = ""
      avg_1st = round(sum(d['1st_alt'] for d in lst) / len(lst))
      avg_2nd = round(sum(d['2nd_alt'] for d in lst) / len(lst))
      avg_odd = round(sum(d['odd'] for d in lst) / len(lst))
      avg_even = round(sum(d['even'] for d in lst) / len(lst))
      avg_total = round(sum(d['total'] for d in lst) / len(lst))
      print(f"{avg_name:<5} | {avg_values:<15} | {avg_1st:>3} | {avg_2nd:>3} | {avg_odd:>3} | {avg_even:>3} | {avg_total:>3}")
  print(sep)
...
# -------------------------------
# SUMMARY OF ALL PREDICTIONS (SEPARATED FOR REUSE IN ANOTHER SCRIPT)
# -------------------------------
print("\n" + "="*50)

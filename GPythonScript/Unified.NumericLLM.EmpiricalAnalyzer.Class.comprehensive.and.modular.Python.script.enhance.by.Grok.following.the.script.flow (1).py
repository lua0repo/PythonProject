# ====================================
# Unified Analytical RNG + Predictive Model
# Combines statistical RNG cracking (Script 1)
# with hybrid predictive modeling (Script 2)
# ====================================
...
# ====================================
# INPUT DATA (20 historical sets)
# ====================================
data = {
    "SET_1": [5, 10, 17, 37, 47, 54],
"SET_2": [1, 13, 17, 20, 34, 46],
"SET_3": [1, 13, 25, 28, 42, 50],
"SET_4": [14, 19, 21, 27, 51, 53],
"SET_5": [28, 31, 42, 44, 45, 54],
"SET_6": [6, 15, 17, 26, 37, 40],
"SET_7": [1, 2, 12, 31, 39, 55],
"SET_8": [4, 22, 23, 24, 38, 46],
"SET_9": [5, 27, 28, 35, 39, 52],
"SET_10": [12, 15, 18, 23, 50, 51],
"SET_11": [2, 22, 26, 39, 42, 44],
"SET_12": [8, 12, 18, 22, 29, 49],
"SET_13": [14, 25, 30, 35, 39, 42],
"SET_14": [3, 13, 19, 41, 48, 49],
"SET_15": [5, 7, 12, 27, 47, 50],
"SET_16": [5, 11, 21, 41, 42, 45],
"SET_17": [2, 20, 30, 48, 49, 51],
"SET_18": [3, 7, 21, 32, 37, 42],
"SET_19": [7, 24, 36, 39, 48, 53],
"SET_20": [5, 10, 12, 13, 16, 36],
}

# ====================================
# RNGCracker and Empirical Analysis
# ====================================
class RNGCracker:
...
def compute_pmp_medians(historical):
    ...
    medians = [round(np.median(pos)) for pos in positions]
    return medians

# ====================================
# DataAnalyzer for statistical summary
# ====================================
class DataAnalyzer:
    ...
    def analyze(self):
        for key, values in self.dataset.items():
            ...
            trend_val = (values[-1]-values[0])/(count-1) if count > 1 else 0
            self.stats[key] = {
                "count": count, "min": min_val, "max": max_val,
                "mean": mean_val, "std": std_val, "trend": trend_val, "values": values
            }
        return self.stats

# ====================================
# Prediction Models (Tabular, NumericLLM, MonteCarlo)
# ====================================
class TabularModel:
    ...
    def predict(self, index):
        s = self.stats[f"SET_{index}"]
        prediction = s["values"][-1] + s["trend"]
        prediction = max(s["min"], min(prediction, s["max"] + s["trend"]))
        reason = f"TabularModel: last={s['values'][-1]}, trend={s['trend']:.2f}, clipped to [{s['min']}, {s['max'] + s['trend']:.2f}]"
        return round(prediction), reason

class NumericLLM:
    def __init__(self, stats):
        self.stats = stats

    def predict(self, index, position):
        prev_sets = [self.stats[f"SET_{i}"]["values"] for i in range(1, index+1)]
        vals = [s[position] for s in prev_sets if position < len(s)]
        if vals:
            avg_val = sum(vals)/len(vals)
            return round(avg_val), f"NumericLLM: position={position}, average of previous sets={avg_val:.2f}"
        return 0, "NumericLLM: no data, default 0"
...
class MonteCarlo:
    ...
    def predict(self, index, position):
        s = self.stats[f"SET_{index}"]
        noise = random.uniform(-s["std"]/4, s["std"]/4)
        prediction = s["values"][-1] + noise
        prediction = max(s["min"], min(prediction, s["max"]))
        reason = f"MonteCarlo: last={s['values'][-1]}, noise={noise:.2f}, clipped to [{s['min']}, {s['max']}]"
        return round(prediction), reason

# ====================================
# Unified Predictor combining models
# ====================================
class UnifiedPredictor:
    def __init__(self, stats):
        self.stats = stats
        self.weights = {"tabular": 0.4, "llm": 0.4, "mc": 0.2}
        self.tabular = TabularModel(stats)
        self.llm = NumericLLM(stats)
        self.mc = MonteCarlo(stats)

    def predict_next_set(self):
        next_set = []
        explanations = []
        next_index = len(self.stats)
        num_positions = len(next(iter(self.stats.values()))["values"])
        for pos in range(num_positions):
            t_val, t_reason = self.tabular.predict(next_index)
            l_val, l_reason = self.llm.predict(next_index, pos)
            mc_val, mc_reason = self.mc.predict(next_index, pos)
            combined = round(t_val*self.weights["tabular"] + l_val*self.weights["llm"] + mc_val*self.weights["mc"])
            all_vals = [v for s in self.stats.values() for v in s["values"]]
            combined = max(min(all_vals), min(combined, max(all_vals)))
            next_set.append(combined)
            explanations.append({
                "position": pos+1,
                "value": combined,
                "reasons": [t_reason, l_reason, mc_reason]
            })
        return next_index+1, next_set, explanations
...
def main():
    rng = RNGCracker(data)
    rng_pred, rng_type, *_ = rng.predict()

    analyzer = DataAnalyzer(data)
    stats = analyzer.analyze()
    predictor = UnifiedPredictor(stats)
    next_index, next_set, explanations = predictor.predict_next_set()

    # REPORT OUTPUT
    print("===== DATA SUMMARY =====")
    for k, s in stats.items():
        print(f"{k}: count={s['count']}, min={s['min']}, max={s['max']}, mean={s['mean']:.2f}, std={s['std']:.2f}, trend={s['trend']:.2f}")

    print("\n===== PREDICTED NEXT SET =====")
    print(f"SET_{next_index}: {next_set}")
   

    print("\n===== REASONING PER ELEMENT =====")
    for exp in explanations:
        print(f"Position {exp['position']}: Predicted={exp['value']}")
        for r in exp["reasons"]:
            print(f"  - {r}")


#===================================
# Universal Pattern Synthesis (UPS) Protocol (Using Basic Libraries) via GeminiA
#===================================
# --- 2. UTILITY AND DATA PREP (Phase I Functions) ---

class Utility:
    """Handles basic math and data restructuring without numpy/pandas."""
   
    @staticmethod
    def extract_columns(data):
        """Restructures dict of sets into 6 positional lists (B1, B2, ...)."""
        num_sets = len(data)
        if num_sets == 0: return [[]] * 6
       
        # Determine number of columns (always 6 in this problem)
        columns = [[] for _ in range(6)]
       
        for key in sorted(data.keys(), key=lambda x: int(x.split('_')[1])):
            set_data = data[key]
            for i in range(6):
                columns[i].append(set_data[i])
        return columns

    @staticmethod
    def calculate_mean(set_data):
        """Calculates the arithmetic mean of a single set."""
        if not set_data: return 0
        return sum(set_data) / len(set_data)

    @staticmethod
    def get_gaps(set_data):
        """Calculates the 5 intra-set gaps."""
        return [set_data[i+1] - set_data[i] for i in range(5)]

    @staticmethod
    def calculate_distance(set1, set2):
        """Calculates simple Euclidean distance between two sets (for K-NN)."""
        return math.sqrt(sum([(set1[i] - set2[i])**2 for i in range(len(set1))]))

# --- 3. FUNCTIONAL MODEL REPLICATION (Phase II Classes) ---

class FunctionalClassifier:
    """Mockup of Classification (M_POS) using K-NN from scratch."""
    ...
    def predict_score(self, target_position):
        """Predicts the normalized probability score for a single position."""
        scores = {n: 0.0 for n in self.all_numbers}
       
        # Use the last set (the one right before the prediction) as the 'query' set
        last_set = self.data_sets[sorted(self.data_sets.keys())[-1]]

        # 1. Find K Nearest Neighbors
        distances = []
        for key, current_set in self.data_sets.items():
            if current_set == last_set: continue # Skip itself
            dist = Utility.calculate_distance(last_set, current_set)
            distances.append((dist, current_set[target_position]))

        distances.sort(key=lambda x: x[0])
        neighbors = distances[:self.k]

        # 2. Score based on neighbor frequency
        # Score is simply the count of the number in the neighbors / K
        for dist, number in neighbors:
            scores[number] += (1 / self.k) # Simple 1/K contribution

        # 3. Normalize scores (Scores already summed up to 1, but we ensure all numbers are covered)
        max_score = max(scores.values()) if scores.values() else 1.0
        if max_score == 0: max_score = 1.0

        return {n: score / max_score for n, score in scores.items()}

class FunctionalRegressor:
    """Mockup of Regression (M_GAP, Constraint) using simple linear average."""
   
    def __init__(self, historical_values):
        self.historical_values = historical_values

    def predict(self):
        """Predicts the next value as the simple historical average."""
        if not self.historical_values: return 0.0
        return sum(self.historical_values) / len(self.historical_values)

# --- 4. THEMATIC & SYNTHESIS LOGIC (Phase II/III Functions) ---

def score_thematic(all_data):
    """
    M_THEME: Implements the Thematic Bias (S_THEME) based on recurrence rules.
    This simulates Rule #6 (Skip-1 Reappearance) and general frequency.
    """
    scores = {n: 0.0 for n in range(1, 59)}
    sets = list(all_data.values())
   
    # 1. Base Score: General Frequency (Higher frequency = higher thematic potential)
    flat_list = [num for sublist in sets for num in sublist]
    for n in flat_list:
        scores[n] += (flat_list.count(n) / len(flat_list)) * 0.5 # 0.5 is weight

    # 2. Skip-1 Repetition Bonus (The strongest thematic rule)
    if len(sets) >= 3:
        set_n_2 = sets[-3] # SET_N-2 (The one skipped)
        for num in set_n_2:
            if num not in sets[-1] and num not in sets[-2]:
                 scores[num] += 0.5 # Add significant bonus for Skip-1 potential
                 
    # Normalize thematic scores
    max_score = max(scores.values()) if scores.values() else 1.0
    return {n: score / max_score for n, score in scores.items()}

def calculate_u_score(s_pos, s_gap, s_theme, weights={'pos': 0.35, 'gap': 0.35, 'theme': 0.30}):
    """Calculates the final Universal Score (U_Score)."""
    u_scores = {}
    for n in range(1, 59):
        # S_POS is the average positional probability (simplified for a single number n)
        avg_pos_score = sum(s_pos[i].get(n, 0.0) for i in range(6)) / 6
       
        # S_GAP score is a derived probability (simplified: higher gap potential = higher score)
        # We simplify S_GAP to be based on number's historical frequency in gap creation
        gap_score = s_gap.get(n, 0.0)
       
        u_scores[n] = (weights['pos'] * avg_pos_score) + \
                      (weights['gap'] * gap_score) + \
                      (weights['theme'] * s_theme.get(n, 0.0))
                     
    return sorted(u_scores.items(), key=lambda item: item[1], reverse=True)

# --- 5. OPTIMIZATION LOGIC (Phase III Execution) ---

def predict_set_24(data):
    """Executes the full UPS Protocol to predict SET_24."""
   
    print("\n--- Phase I: Data Preprocessing ---")
    columns = Utility.extract_columns(data)
    all_sets = list(data.values())
   
    # 1. Predict Constraint (mu_pred)
    historical_means = [Utility.calculate_mean(s) for s in all_sets]
    regressor = FunctionalRegressor(historical_means)
    mu_pred = regressor.predict()
    print(f"Predicted Global Mean (mu_pred): {mu_pred:.2f}")


    # 2. Predict Gaps (for S_GAP and Final Selection)
    historical_gaps = [Utility.get_gaps(s) for s in all_sets]
    predicted_gaps = [FunctionalRegressor([g[i] for g in historical_gaps]).predict() for i in range(5)]
    print(f"Predicted Gaps: {[f'{g:.1f}' for g in predicted_gaps]}")
   
    # 3. M_POS Scoring (Simulated Positional Probability)
    s_pos_scores = [FunctionalClassifier(data).predict_score(i) for i in range(6)]
    print("M_POS (Structural) Scores Generated.")

    # 4. M_THEME Scoring (Simulated Thematic Bias)
    s_theme_scores = score_thematic(data)
    print("M_THEME (Thematic Bias) Scores Generated.")

    # 5. M_GAP Scoring (Simplified: S_GAP = Positional Average of Gap Magnitude)
    # A simplified S_GAP score based on a number's historical tendency to follow a small gap
    s_gap_scores = {}
    for n in range(1, 59):
        # Score is based on how often a number is B_i where gap B_i - B_i-1 is small (e.g., < 5)
        small_gap_counts = 0
        for s in all_sets:
            for i in range(1, 6):
                if s[i] == n and (s[i] - s[i-1]) <= 5:
                    small_gap_counts += 1
        s_gap_scores[n] = small_gap_counts / len(all_sets)
    print("M_GAP (Dynamics) Scores Generated.")

    print("\n--- Phase II & III: Synthesis and Optimization ---")
   
    # Calculate U_Score and rank all 58 numbers
    u_score_ranking = calculate_u_score(s_pos_scores, s_gap_scores, s_theme_scores)
   
    # --- 1. Core Selection (Top 4) ---
    core_4 = sorted([n for n, score in u_score_ranking[:4]])
   
    print(f"Top 10 U-Score Candidates: {[n for n,s in u_score_ranking[:10]]}")
    print(f"Core 4 Selection: {core_4}")

    # --- 2. Optimization Selection (Final 2) ---
   
    # Candidates for the last 2 spots (ranks 5-10)
    optimization_candidates = [n for n, score in u_score_ranking[4:10]]
   
    best_set = []
    min_mean_error = float('inf')
    best_local_score = -1
   
    # Iterate through all combinations of 2 candidates
    for i in range(len(optimization_candidates)):
        for j in range(i + 1, len(optimization_candidates)):
            pair = [optimization_candidates[i], optimization_candidates[j]]
            current_set = sorted(core_4 + pair)
           
            # --- PRIORITY 1: Maximize Local Structural Fit (The FIX) ---
            # Score based on how small the gaps are (tight clustering)
            current_gaps = Utility.get_gaps(current_set)
            local_score = sum(1 for g in current_gaps if g <= 5) # Score 1 point for every small gap
           
            # --- PRIORITY 2: Minimize Global Mean Error ---
            current_mean = Utility.calculate_mean(current_set)
            mean_error = abs(current_mean - mu_pred)

            # Selection Logic: Prioritize local_score first, then mean_error
            if local_score > best_local_score:
                # New best local fit found
                best_local_score = local_score
                best_set = current_set
                min_mean_error = mean_error
            elif local_score == best_local_score and mean_error < min_mean_error:
                # Same local fit, but better mean error
                best_set = current_set
                min_mean_error = mean_error
               
    # Check Odd/Even Parity (Simplified: just check for a 3-3 split which is common)
    odd_count = sum(1 for n in best_set if n % 2 != 0)
    if odd_count not in [2, 3, 4]: # If parity is extreme, try reversing the last pair (basic reversal)
        shuffle(best_set)
        best_set.sort()
       
    print(f"Best Local Score (Small Gaps): {best_local_score}")
    print(f"Final Set Mean Error: {min_mean_error:.2f}")

    return sorted(best_set)

# --- EXECUTION ---

final_prediction = predict_set_24(data)

print("\n---------------------------------------------------")
print(f"✨ Final Predicted Set 24 (UPS Protocol): {final_prediction}")
print("---------------------------------------------------\n")
...
#=====================================
#[Systematical] comprehensive and modular Python script enhance by Grok following the script flow ***START***
#=====================================
...
def print_set_analysis(data_list):
   ...
   header = (
       f"{'Set Name':<5} | {'Set Values':<10} | {'1st Alt':>5} | "
       f"{'2nd Alt':>5} | {'Odd':>5} | {'Even':>5} | {'Total':>5}"
   )
   ...
   for item in data_list:
       ...
       try:
           ...
           row = (
               f"{item['name']:<5} | {values_str:<10} | {item['1st_alt']:>3} | "
               f"{item['2nd_alt']:>3} | {item['odd']:>3} | {item['even']:>5} | "
               f"{item['total']:>3}"
           )
           print(row)
...
#=====================================
#[Systematical] comprehensive and modular Python script enhance by Grok following the script flow ***END***
#=====================================

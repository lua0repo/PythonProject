...
# -------------------------------
historical_data = {
 "SET_1": [8, 12, 18, 22, 29, 49],
"SET_2": [14, 25, 30, 35, 39, 42],
"SET_3": [3, 13, 19, 41, 48, 49],
"SET_4": [5, 7, 12, 27, 47, 50],
"SET_5": [5, 11, 21, 41, 42, 45],
"SET_6": [2, 20, 30, 48, 49, 51],
"SET_7": [3, 7, 21, 32, 37, 42],
"SET_8": [7, 24, 36, 39, 48, 53],
"SET_9": [5, 10, 12, 13, 16, 36],
"SET_10": [7, 11, 14, 21, 30, 52],
}
...
class DifferencePredictor:
    ...
    def display_differences(self):
        ...
        print("-" * 40)
        # Additional: Compute and print cumulative difference from first to last as "AVERAGE RESULT"
        first = self.df.iloc[0].values
        last = self.df.iloc[-1].values
        avg_res = [float(int(round(float(last[i] - first[i])))) for i in range(6)]
        self.diff_df.loc['AVERAGE RESULT'] = avg_res
        print(self.diff_df.to_string())
        self.diff_df = self.diff_df.drop('AVERAGE RESULT') # Remove to keep original
    def predict_diffs_regression(self):
        ...
        for col in self.diff_df.columns:
            ...
            reg.fit(time_steps, y)
            next_time = len(self.diff_df) + 1
            pred = reg.predict(next_time)
            predicted_diff.append(round(pred))
...
predicted_diff = predictor_model.predict_diffs_regression()
print("\nPredicted Difference from Last SET (ML Regression) =>", ' '.join(map(str, predicted_diff)))
last_set = [int(x) for x in df.iloc[-1].tolist()]
pre_Deff_set = sorted([max(1, min(58, int(val + diff))) for val, diff in zip(last_set, predicted_diff)])  # Bound to 1-58
...
# -------------------------------
# CUSTOM KNN-LIKE PREDICTOR (Enhanced with weighting)
# -------------------------------
...
# Enhanced version with recency weighting
class WeightedKNNPredictor(HistoricalKNNPredictor):
    def __init__(self, k=3, recency_weight=0.5):
        super().__init__(k)
    ...
    def _get_nearest_neighbors(self, input_profile):
        ...
        distances.sort(key=lambda x: x[0])
        return [s for _, s in distances[:self.k]]
weighted_knn = WeightedKNNPredictor(k=5, recency_weight=0.6)
weighted_knn.fit(even_odd_profiles, all_sets)
pre_EandO_set = weighted_knn.predict(last_profile)
print(f"✅Predicted E & O => {pre_EandO_set}")
# -------------------------------
...
def calculate_set_stats(name, values):
    odd = len([n for n in values if n % 2 != 0])
    even = len([n for n in values if n % 2 == 0])
    ...
    return {"name":name, "values":values, "1st_alt":a1, "2nd_alt":a2,
            "odd":odd, "even":even, "total":total}
def print_set_analysis(lst):
    if not lst: return
    header = f"{'Set Name':<5} | {'Set Values':<15} | {'1st Alt':>3} | {'2nd Alt':>3} | {'Odd':>3} | {'Even':>3} | {'Total':>3}"
    sep = "="*len(header)
    print(sep); print(header); print(sep)
    for d in lst:
        vs = ", ".join(map(str,d['values']))
        print(f"{d['name']:<5} | {vs:<15} | {d['1st_alt']:>3} | {d['2nd_alt']:>3} | {d['odd']:>3} | {d['even']:>3} | {d['total']:>3}")
    ...
    if lst:
        ...
        stats = { '1st_alt': [d['1st_alt'] for d in lst], '2nd_alt': [d['2nd_alt'] for d in lst],
                  'odd': [d['odd'] for d in lst], 'even': [d['even'] for d in lst], 'total': [d['total'] for d in lst] }
        pred_stats = {}
        for key, vals in stats.items():
            reg = CustomLinearRegression()
            reg.fit(time_steps, vals)
            pred_stats[key] = round(reg.predict(len(lst) + 1))
        print(f"{pred_name:<5} | {pred_values:<15} | {pred_stats['1st_alt']:>3} | {pred_stats['2nd_alt']:>3} | {pred_stats['odd']:>3} | {pred_stats['even']:>3} | {pred_stats['total']:>3}")
    print(sep)
...
print("-" * 40)
print(f"✅ Ensemble Majority Vote: {ensemble_vote}")
# -------------------------------
...
print(f"Predicted numbers : {ups_pred}")
print(f"FINAL PREDICTION SET_11: {pred_set}")
print(f"✅ Best Predicted Set: {' '.join(map(str, best_predicted_set))}")

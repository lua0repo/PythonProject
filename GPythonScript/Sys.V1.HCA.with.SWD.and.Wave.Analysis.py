...
COMPLETE_HISTORICAL_DATA = {
    "SET_1": [15, 27, 40, 45, 48, 58],
    "SET_2": [6, 14, 18, 19, 43, 49],
    "SET_3": [1, 12, 14, 35, 37, 43],
    "SET_4": [2, 7, 23, 31, 35, 50],
    "SET_5": [8, 14, 17, 42, 49, 51],
    "SET_6": [6, 11, 12, 13, 43, 54],
    "SET_7": [4, 17, 19, 28, 46, 50],
    "SET_8": [3, 12, 23, 28, 31, 44],
    "SET_9": [13, 20, 37, 49, 51, 56],
    "SET_10": [17, 24, 30, 44, 53, 56],
}
# ===========================
...
next_counts_list = []
previous_raw_lines = []  # Added for integration
...
for window_idx in range(num_windows):
    ...
    if window_idx + window_size < len(keys):
        ...
        actual_freqs, count_str_line = results['num_predictor'].validate_actual_set(actual_next_key, actual_next_set, window_idx + 1, results['predicted_pos_means'], results['pre_EandO_set_knn'])
...
# =====================================================
# INTEGRATED NEW SCRIPT
# =====================================================
from collections import defaultdict
import statistics
# Set raw inputs from collected data
raw_previous_sets = previous_raw_lines
last_results = all_window_results[-1]
max_num = last_results['max_number']
freq_count = last_results['frequency_count']
raw_current_availability = ' '.join(f"{i} = {freq_count[i]}" for i in range(1, max_num + 1))
# =====================================================
# STEP 2: PARSERS (AUTOMATIC CONVERSION)
# =====================================================
...
def parse_current_availability(raw_text):
    ...
    for i in range(0, len(tokens), 3):
        ...
        result[number] = value
    return result
previous_sets = [parse_previous_set(line) for line in raw_previous_sets]
current_availability = parse_current_availability(raw_current_availability)
# =====================================================
# STEP 3: BUILD HISTORY
# =====================================================
history = defaultdict(list)
for s in previous_sets:
    for number, value in s.items():
...
top_two = [a["number"] for a in analysis[:2]]
top_six = [a["number"] for a in analysis[:6]]
print("\nRECOMMENDED FIRST TWO ELEMENTS:")
print(top_two)
print("\nOPTIONAL NEXT SET OF 6 ELEMENTS:")
print(top_six)

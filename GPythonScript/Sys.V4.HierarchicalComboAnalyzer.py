...
# -------------------------------
historical_data = {
"SET_1": [1, 8, 18, 46, 52, 53],
"SET_2": [5, 8, 12, 29, 39, 44],
"SET_3": [4, 11, 17, 25, 37, 52],
"SET_4": [3, 6, 30, 42, 45, 49],
"SET_5": [11, 12, 17, 29, 40, 48],
"SET_6": [17, 23, 30, 43, 45, 48],
"SET_7": [6, 7, 28, 32, 42, 45],
"SET_8": [5, 26, 28, 35, 41, 48],
"SET_9": [6, 9, 14, 21, 39, 45],
"SET_10": [13, 15, 24, 35, 41, 45],
}
...
class LotteryAnalyzerUnified:
    ...
    def generate_report(self, pred, scores, ga_pred):
        ...
        for n,f,p,pr,prng,ga,tot in comp:
            lines.append(f"{n:<2} | {f:<2} | {p:<2.0f} | {pr:<2} | {prng:<.1f} {ga:<2} | {tot:.1f}")
        lines.append("\nFINAL PREDICTION")
...
def print_set_analysis(lst):
    if not lst: return
    header = f"{'Set Name':<2} | {'Set Values':<4} | {'1st Alt':>2} | {'2nd Alt':>2} | {'Odd':>2} | {'Even':>2} | {'COdd':>2} | {'CEven':>2} | {'Total':>2}"
    sep = "="*len(header)
    print(sep); print(header); print(sep)
    for d in lst:
        vs = ", ".join(map(str,d['values']))
        print(f"{d['name']:<2} | {vs:<4} | {d['1st_alt']:>2} | {d['2nd_alt']:>2} | {d['odd']:>2} | {d['even']:>2} | {d['codd']:>2} | {d['ceven']:>2} | {d['total']:>2}")
    ...
    if lst:
        ...
        print(f"{pred_name:<2} | {pred_values:<4} | {pred_stats['1st_alt']:>2} | {pred_stats['2nd_alt']:>2} | {pred_stats['odd']:>2} | {pred_stats['even']:>2} | {pred_stats['codd']:>2} | {pred_stats['ceven']:>2} | {pred_stats['total']:>2}")
    print(sep)
...
print(f"✅📝Predicted numbers : {ups_pred}")
print(f"✅G for 20/60 SET3️⃣🎯⭐FINAL PREDICTION SET_11: {pred_set}")
print(f"✅G for 60 SET3️⃣🎯Best Predicted Set: {' '.join(map(str, best_predicted_set))}")

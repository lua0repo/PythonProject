...
for type_name, historical_data in data_types.items():
    ...
    # Determine if sorting should be disabled for this type
    DISABLE_SORTING = type_name in ["Type1", "Type2", "Type3"]
   
    if DISABLE_SORTING:
        print("⚠️  SORTING DISABLED FOR THIS TYPE - Preserving original order")
    ...
    class DifferencePredictor:
        ...
        def display_differences(self):
            ...
            pre_Deff_set = [max(min_number, min(max_number, last_set[i] + predicted_diff[i])) for i in range(n_balls)]
           
            # Apply sorting only for Type4 and Type5
            if not DISABLE_SORTING:
                pre_Deff_set = sorted(pre_Deff_set)
    ...
    pre_EandO_set = random.sample(hot_evens, min(pred_even_count, len(hot_evens))) + random.sample(hot_odds, min(pred_odd_count, len(hot_odds)))
   
    # Apply sorting only for Type4 and Type5
    if not DISABLE_SORTING:
        pre_EandO_set = sorted(pre_EandO_set)
    ...
    class HistoricalKNNPredictor:
        ...
        def predict(self, input_profile):
            ...
            predicted_set = [num for num, _ in most_common]
            if not DISABLE_SORTING:
    ...
    class EnhancedKNNPredictor(HistoricalKNNPredictor):
        ...
        def predict(self, input_profile):
            ...
            while len(predicted) < n_balls:
                predicted.append(predicted[-1] + 1 if predicted else min_number)
    ...
    # Don't sort for Type1, Type2, Type3
    prediction_AN = predicted_pos_means if DISABLE_SORTING else sorted(predicted_pos_means)
    ...
    def knn_predict_combination(historical_data_dict, k=3):
        ...
        for s in sets_list[:-1]:
            s_sorted = sorted(s)
            dist = euclidean(last_set, s_sorted)
        ...
        predicted_knn = list(set(common_pairs_knn))
        if not DISABLE_SORTING:
            predicted_knn = sorted(predicted_knn)
        predicted_knn = predicted_knn[:n_balls]
    ...
    def ml_predict_combination(pair_counter):
        ...
        predicted_ml = list(set(common_nums_ml))
        if not DISABLE_SORTING:
            predicted_ml = sorted(predicted_ml)
        predicted_ml = predicted_ml[:n_balls]
    ...
    predicted_combination_seg = list(set(predicted_combination_seg))
    if not DISABLE_SORTING:
        predicted_combination_seg = sorted(predicted_combination_seg)
    ...
    scored_candidates = [(score_candidate(s, last_draw, hot_numbers_set, cold_numbers_set), s)
                        for s in expanded_candidate_sets if len(s) >= n_balls]
    ...
    class RNGCracker:
        ...
        def crack_mt(self):
            if len(self.flatten_seq) >= n_balls * 100:
                return None
            np.random.seed(20251110)
            candidate = np.random.choice(range(min_number + 1 if min_number == 0 else min_number, max_number + 1), n_balls, replace=False)
            if not DISABLE_SORTING:
    ...
    class Utility:
        ...
        @staticmethod
        def get_gaps(s):
            return [s[i+1] - s[i] for i in range(len(s)-1)] if len(s) >= 2 else []
    ...
    def calculate_u_score(s_pos, s_gap, s_theme, w={'pos':0.35, 'gap':0.35, 'theme':0.30}):
        u = {}
    ...
    def predict_set_24(historical_data_dict):
        ...
        # Don't sort for Type1, Type2, Type3
        core = [n for n, _ in u_rank[:core_size]]
        if not DISABLE_SORTING:
            core = sorted(core)
    ...
    class LotteryAnalyzerUnified:
        ...
        def validate_and_organize(self):
            ...
            for i in range(1, self.total_sets + 1):
                ...
                if len(s) != n_balls:
                    raise ValueError(f"{key} must have {n_balls} numbers")
        ...
        def genetic_algorithm_prediction(self):
            def gen():
                result = random.sample(range(self.global_min, self.global_max + 1), n_balls)
                if not DISABLE_SORTING:
        ...
        def predict_next_set(self):
            ...
            for n, c in self.features['frequency'].items():
                scores[n] += c * 2
           
            for pos in self.features['positional_freq']:
                for n, c in self.features['positional_freq'][pos].items():
                    scores[n] += c * 1.5
    ...
    def print_set_analysis(lst):
        ...
        if lst:
            ...
            stats = {
                '1st_alt': [d['1st_alt'] for d in lst],
                '2nd_alt': [d['2nd_alt'] for d in lst],
                'odd': [d['odd'] for d in lst],
                'even': [d['even'] for d in lst],
                'codd': [d['codd'] for d in lst],
                'ceven': [d['ceven'] for d in lst],
                'total': [d['total'] for d in lst]
            }
    ...
    class EnsemblePredictor:
        ...
        def predict(self, top_n=n_balls, method='majority_vote'):
            ...
            if method == 'majority_vote':
                ...
                ensemble_set = [num for num, _ in counts.most_common(top_n)]
                if not DISABLE_SORTING:
                    ensemble_set = sorted(ensemble_set)
    ...
    # ============================================
    # ZERO-FREQUENCY ELEMENT ANTICIPATOR
    # ============================================
    ...
    # ============================================
    # BALANCED FREQUENCY ANTICIPATION
    # ============================================

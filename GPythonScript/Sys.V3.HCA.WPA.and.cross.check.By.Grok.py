...
# -------------------------------
historical_data = {
   "SET_1": [6, 11, 12, 13, 43, 54],
 "SET_2": [4, 17, 19, 28, 46, 50],
 "SET_3": [3, 12, 23, 28, 31, 44],
 "SET_4": [13, 20, 37, 49, 51, 56],
 "SET_5": [17, 24, 30, 44, 53, 56],
 "SET_6": [19, 20, 37, 50, 51, 52],
 "SET_7": [3, 8, 12, 30, 34, 54],
 "SET_8": [15, 23, 30, 33, 53, 57],
 "SET_9": [9, 31, 32, 35, 44, 51],
 "SET_10": [11, 28, 33, 43, 44, 45],
 "SET_11": [5, 15, 17, 19, 35, 37],
 "SET_12": [2, 3, 22, 38, 55, 56],
 "SET_13": [5, 20, 22, 49, 52, 54],
 "SET_14": [8, 13, 19, 21, 25, 33],
 "SET_15": [2, 4, 9, 11, 37, 39],
 "SET_16": [6, 7, 25, 27, 46, 50],
 "SET_17": [2, 43, 44, 45, 49, 57],
 "SET_18": [17, 20, 23, 28, 30, 55],
 "SET_19": [12, 13, 31, 35, 40, 55],
 "SET_20": [6, 11, 28, 34, 37, 50],
}
...
Bmost_frequent_numbers = [nums[0] for nums in top_1 if nums]  # Fixed: Check if nums is not empty
...
def print_set_analysis(lst):
    if not lst: return
    header = f"{'Set Name':<10} | {'Set Values':<25} | {'1st Alt':>7} | {'2nd Alt':>7} | {'Odd':>5} | {'Even':>5} | {'COdd':>5} | {'CEven':>5} | {'Total':>5}"
    sep = "="*len(header)
    print(sep); print(header); print(sep)
    for d in lst:
        vs = ", ".join(map(str,d['values']))
        print(f"{d['name']:<10} | {vs:<25} | {d['1st_alt']:>7} | {d['2nd_alt']:>7} | {d['odd']:>5} | {d['even']:>5} | {d['codd']:>5} | {d['ceven']:>5} | {d['total']:>5}")
...
class ComprehensiveSetAnalyzer:
    """
    Analyzes any 6-number set against all prediction methods
    ...
    def _build_all_methods(self):
        ...
        methods = {}
        # Use all the predictions already generated in the script
    ...
    def analyze(self, candidate_set, set_name="User Provided Set"):
        ...
        """
        if len(candidate_set) != 6:
            print(f"❌ Error: Set must contain exactly 6 numbers, got {len(candidate_set)}")
            return
        ...
        for num in candidate_set:
            ...
            percentage = round((count / len(self.all_methods)) * 100, 1)
            if count == 0:
...
def backtest_hits(historical_data, predictions, holdout_start=10):
    """Backtest: For each holdout set (from holdout_start onward), count hits (common numbers) with the prediction.
    Simulates cross-val: Higher avg hits = better support/accuracy."""
    sets = list(historical_data.values())
    hits = {}
    for method, pred in predictions.items():
        method_hits = []
        for i in range(holdout_start, len(sets)):
            # In full dynamic version, refit method on sets[0:i] here; for simplicity, use fixed pred
            actual = sets[i]
            common = len(set(pred) & set(actual))
            method_hits.append(common)
        avg_hit = sum(method_hits) / len(method_hits) if method_hits else 0
        hits[method] = avg_hit
    return hits
def compute_overlaps(predictions):
    """Compute pairwise overlaps (# common numbers) as support metric.
    High overlap = methods validate/support each other."""
    overlaps = {}
    for m1, m2 in combinations(predictions.keys(), 2):
        key = f"{m1}-{m2}"
        overlap = len(set(predictions[m1]) & set(predictions[m2]))
        overlaps[key] = overlap
    return overlaps
def find_best(predictions, backtest_hits, overlaps, weights={'hits':0.5, 'overlap':0.3, 'diversity':0.2}):
    """Identify best: Score = weighted (hits accuracy + avg_overlap support + diversity compensation).
    Diversity = 6 - avg_overlap (rewards unique coverage to compensate biases)."""
    scores = {}
    for method in predictions:
        method_overlaps = [v for k, v in overlaps.items() if method in k.split('-')]
        avg_overlap = sum(method_overlaps) / len(method_overlaps) if method_overlaps else 0
        diversity = 6 - avg_overlap # Compensate: High if method adds unique numbers
        score = (weights['hits'] * backtest_hits.get(method, 0) +
                 weights['overlap'] * avg_overlap +
                 weights['diversity'] * diversity)
        scores[method] = score
    best_method = max(scores, key=scores.get)
    return best_method, scores[best_method], predictions[best_method]
# Run the analysis
hits = backtest_hits(historical_data, all_predictions)
overlaps = compute_overlaps(all_predictions)
best_method, best_score, best_pred = find_best(all_predictions, hits, overlaps)
# Output reasoning and results
print("\n=== Reasoning and Cross-Validation Analysis ===")
print("Backtest Average Hits (Support via Accuracy):")
print("Higher = method predicts historical holdouts well (validates reliability).")
print(json.dumps(hits, indent=2))
print("\nPairwise Overlaps (Method Agreement/Support):")
print("Higher = methods support each other (e.g., agree on hot numbers).")
print(json.dumps(overlaps, indent=2))
print("\nBest Method Identification:")
print(f"Best Method: {best_method} with Score {best_score:.2f}")
print(f"Best Prediction: {best_pred}")
print("Reasoning: Score balances hits (accuracy support from cross-val), overlap (mutual validation), ")
print("and diversity (compensation for biases, e.g., adding unique numbers from trend methods).")
print("Adjust weights if needed (e.g., more on hits for accuracy focus).")
print("For dynamic use: Update historical_data with new draws, rerun for fresh val.")
